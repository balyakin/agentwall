...
func TestGenericSecretAssignmentRedaction(t *testing.T) {
	s, err := New(Config{Enabled: true, MaxBodyBytes: 4096})
	if err != nil {
		t.Fatal(err)
	}
	in := []byte("secret_key=AbcDefGh1234567890")
	out, ids := s.RedactBytes(in, false)
	if bytes.Contains(out, []byte("AbcDefGh1234567890")) {
		t.Fatalf("expected generic secret assignment value to be redacted")
	}
	matched := false
	for _, id := range ids {
		if id == "generic_secret_assignment" {
			matched = true
			break
		}
	}
	if !matched {
		t.Fatalf("expected generic_secret_assignment sanitizer id")
	}
}

func TestGoogleAPIKeyRedaction(t *testing.T) {
	s, err := New(Config{Enabled: true, MaxBodyBytes: 4096})
	if err != nil {
		t.Fatal(err)
	}
	in := []byte(`{"google_api_key":"AIzaSyA-BCDEFGHIJKLMNOPQRSTUVWXYZ123456"}`)
	out, ids := s.RedactBytes(in, false)
	if bytes.Contains(out, []byte("AIzaSyA-BCDEFGHIJKLMNOPQRSTUVWXYZ123456")) {
		t.Fatalf("expected google api key to be redacted")
	}
	matched := false
	for _, id := range ids {
	...
	if !matched {
		t.Fatalf("expected google_api_key sanitizer id")
	}
}
